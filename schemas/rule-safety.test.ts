...
test("the pattern that hung the scanner is rejected", () => {
  ...
  const verdict = checkRegexSafety("(\\w+\\s?)+=")
  assert.equal(verdict.ok, false)
  assert.match(verdict.ok === false ? verdict.reason : "", /exponential/)
})

test("classic catastrophic backtracking is rejected", () => {
  assert.equal(checkRegexSafety("(a+)+$").ok, false)
})

test("every rule Pushguard ships is accepted", () => {
  ...
  for (const rule of defaultRules) {
    for (const source of [rule.added_lines, rule.commit_message]) {
      if (!source) continue
      assert.equal(checkRegexSafety(source).ok, true, `${rule.id}: /${source}/ was rejected`)
    }
  }
})

test("a rule carrying an unsafe regex cannot be saved", () => {
  const result = checkedRuleSchema.safeParse({
...
test("commit_message is checked too, not only added_lines", () => {
  const result = checkedRuleSchema.safeParse({
